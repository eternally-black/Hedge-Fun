...
2. **DNS** — A records `glitchtip.hedgeyour.fun` and `kuma.hedgeyour.fun` → **VPS2**.
3. **VPS2 install** — `scp -r ops root@<vps2>:/root/hedgefun-ops && ssh root@<vps2> bash
   ...
   Kuma admin + monitors; fill `/opt/ops/.env`; install the printed `backup_pull`
...
6. **Watcher-of-the-watcher** — one free UptimeRobot monitor on
   `https://kuma.hedgeyour.fun` with Telegram **and email** notifications.
