...
3. On VPS1, once:
   ...
   and append to `/var/lib/postgresql/data/pg_hba.conf` **inside the db container** (the file
   is generated by initdb and lives in the volume, so it survives recreation but is not in
   this repo), then `SELECT pg_reload_conf();`:
   `host replication replicator 172.16.0.0/12 scram-sha-256`
   The range is the docker container range, not one address: bridge subnets get new
   addresses whenever a network is recreated, and the tunnel makes the connection appear to
   come from the bridge gateway. The port is loopback-only and the password does the work.
