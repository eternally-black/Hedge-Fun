...
// Onboarding / auth overlay (ported from app design). Both buttons open the same
// Privy login() — Privy's modal offers email + X (both enabled in the dashboard).
  ...
  return (
    <div style={{ position: "absolute", inset: 0, zIndex: 90, background: "radial-gradient(120% 80% at 50% 0%, color-mix(in srgb,var(--energy) 26%,transparent), transparent 55%), var(--bg)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 32, textAlign: "center" }}>
      ...
      <div style={{ width: "100%", maxWidth: 330, marginTop: 34, display: "flex", flexDirection: "column", gap: 10 }}>
        <button type="button" onClick={onLogin} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, background: "#fff", color: "#111", fontWeight: 700, padding: 15, borderRadius: 16, cursor: "pointer", fontSize: 15, border: "none", width: "100%" }}>
          𝕏 Continue with Twitter
        </button>
        <button type="button" onClick={onLogin} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, background: "var(--panel)", color: "var(--text)", fontWeight: 700, padding: 15, borderRadius: 16, cursor: "pointer", border: "1px solid var(--line)", fontSize: 15, width: "100%" }}>
          ✉ Continue with Email
        </button>
      </div>
