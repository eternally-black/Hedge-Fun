...
export const StockCardFace = memo(function StockCardFace({ card, yesP, noP, skipP, stakeCents, onPickStake, realMode }: FaceProps) {
      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", padding: "16px 18px 18px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
          ...
          {!card.tradable ? (
            ...
            <div style={{ display: "flex", alignItems: "center", background: "rgba(0,0,0,.4)", backdropFilter: "blur(6px)", padding: "6px 11px", borderRadius: 20, border: "1px solid color-mix(in srgb,var(--gold) 45%,transparent)" }}>
              <span style={{ fontSize: 10, letterSpacing: ".1em", textTransform: "uppercase", fontWeight: 700, color: "var(--gold)" }}>Real</span>
            </div>
          ) : null}
        </div>
        ...
        <StakeChips stakeCents={stakeCents} onPickStake={onPickStake} real={spendsReal} />
...
function StakeChips({ stakeCents, onPickStake, real }: { stakeCents: number; onPickStake: (c: number) => void; real: boolean }) {
      ...
      {STOCK_STAKE_PRESETS_CENTS.map((c) => {
        ...
        return (
          ...
          >
            ...
            <div style={{ fontSize: 8, letterSpacing: ".12em", color: real ? "var(--gold)" : "var(--muted)", textTransform: "uppercase" }}>{real ? "Real" : "Paper"}</div>
          </button>
        );
      })}
...
// ============================================================================
export const StockCardPreview = memo(function StockCardPreview({ card, stakeCents, realMode }: { card: StockDeckCard; stakeCents: number; realMode: boolean }) {
  return (
    <div style={{ position: "absolute", inset: 0, borderRadius: 26, overflow: "hidden", background: "var(--panel2)", border: "1px solid var(--line)", filter: "brightness(.82)", pointerEvents: "none", transform: `scale(${PREVIEW_SCALE}) translateY(${PREVIEW_Y}px)`, transformOrigin: "center bottom" }}>
      <StockCardFace card={card} yesP={0} noP={0} skipP={0} stakeCents={stakeCents} onPickStake={NOOP} realMode={realMode} />
    </div>
  );
});
...
export function StockDeckCard({
  ...
  return (
    <SwipeShell busy={busy} onAction={onAction} onTap={() => {}}>
      {({ yesP, noP, skipP }) => (
        <StockCardFace card={card} yesP={yesP} noP={noP} skipP={skipP} stakeCents={stakeCents} onPickStake={onPickStake} realMode={realMode} />
      )}
    </SwipeShell>
  );
}
