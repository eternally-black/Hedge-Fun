...
// --- windowed scoring: filtering by utcDay then scorePoints is the route's contract -----------
{
  ...
  assert.strictEqual(scorePoints(rows).total, 3, "all-time = 3");
  assert.strictEqual(scorePoints(rows.filter((r) => r.utcDay === today)).total, 1, "today = 1");

  const weekCutoff = new Date(Date.now() - 6 * 86_400_000).toISOString().slice(0, 10);
  assert.strictEqual(
    scorePoints(rows.filter((r) => r.utcDay >= weekCutoff)).total,
    1,
